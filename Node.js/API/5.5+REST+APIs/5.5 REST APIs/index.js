...
const yourBearerToken = "a9ba1a97-2fee-4b20-b083-a3a5c5a8fda9";
const config = {
...
app.post("/post-secret", async (req, res) => {
  ...
  try{
    ...
    res.render("index.ejs", { content : JSON.stringify(result.data)});
    console.log("post secret succesfully!");
  }catch (err) {
    console.log(err);
...
app.post("/patch-secret", async (req, res) => {
  ...
  try {
    const result = await axios.patch(`${API_URL}/secrets/${searchId}`, {
      secret : req.body.secret,
      score : req.body.score,        //====> 이 객체를 req.body로 교체 가능 그게 더 코드가 간결화 됨 
    }, config);
    res.render("index.ejs", { content : JSON.stringify(result.data)});
  } catch (err) {
    console.log(err);
